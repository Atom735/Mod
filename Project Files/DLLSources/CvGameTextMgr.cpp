...
void createTestFontString(CvWStringBuffer& szString)
{
	...
	szString.append(L"{}~\\????G????T??????????S??F?O????????a??de??????µ???p??st?f???????????«»°???????©®?Ј??????");
	szString.append(L"\n");
...
void CvGameTextMgr::setBuildingHelp(CvWStringBuffer &szBuffer, BuildingTypes eBuilding, bool bCivilopediaText, bool bStrategyText, CvCity* pCity)
{
	...
	// RU: Пункт увелечения размера хранилища.
	...
	if (iYieldStorage > 0)
	{
		...
		// Calculate actual adding.
		if (pCity != NULL) {
			int iYieldStorageNew = pCity->calculateMaxYieldCapacityWithBuilding(eBuilding);
			if (iYieldStorage != iYieldStorageNew) {
				szTempBuffer.Format(SETCOLR L" (%+d)" ENDCOLR, TEXT_COLOR("COLOR_BUILDING_TEXT"), iYieldStorageNew - iYieldStorage);
				szBuffer.append(szTempBuffer);
			}
		}
